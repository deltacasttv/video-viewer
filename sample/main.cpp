...
int main(int argc, char** argv) 
{
   ...
   std::thread viewerthread(render_video, std::ref(viewer), 1920, 1080, "My window", texture_width, texture_height, Deltacast::VideoViewer::InputFormat::bgr_444_8, 10, std::ref(stop), std::ref(synchronisation_cv), std::ref(synchronisation_mutex));
#else
   if(! viewer.init(800, 600, "My window", texture_width, texture_height, Deltacast::VideoViewer::InputFormat::ycbcr_422_10_le_msb))
   {
      std::cout << "VideoViewer initialization failed" << std::endl;
      return -1;
   }
