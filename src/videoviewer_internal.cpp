...
#define GL_CHECK(stmt) do { \
            stmt; \
            PrintGLError(#stmt, __FILE__, __LINE__); \
        } while (0)
#define GLFW_CHECK(stmt) do { \
            stmt; \
            PrintGLFWError(#stmt, __FILE__, __LINE__); \
        } while (0)
#else
#define GL_CHECK(stmt) stmt
#define GLFW_CHECK(stmt) stmt
#endif
...
void VideoViewer_Internal::PrintGLError(const char* stmt, const char* fname, int line)
{
   ...
   err = glGetError();
   if(err != GL_NO_ERROR)
      std::cout << "Opengl error: " << err << " [" << stmt  << "]" << "in file " << fname << " @ line " << line << std::endl;
}

void VideoViewer_Internal::PrintGLFWError(const char* stmt, const char* fname, int line)
{
...
void VideoViewer_Internal::compileShader(GLuint shaderID, const char* sourcePointer) {
   GL_CHECK(glShaderSource(shaderID, 1, &sourcePointer, NULL));
   ...
   GL_CHECK(glGetShaderInfoLog(shaderID, InfoLogLength, NULL, shaderErrorMessage));
...
bool VideoViewer_Internal::create_window(int width, int height, const char* title, const bool is_invisible)
{
   ...
   m_window = GLFW_CHECK(glfwCreateWindow(width, height, title, NULL, NULL));
   ...
   GLFW_CHECK(glfwMakeContextCurrent(NULL));
...
void VideoViewer_Internal::create_shaders(const char* compute_shader_name)
{
   ...
   GL_CHECK(glGetProgramInfoLog(m_programID, info_log_length, NULL, &program_error_msg[0]));
...
void VideoViewer_Internal::create_vertexes()
{
   ...
   GL_CHECK(glBufferData(GL_ARRAY_BUFFER, sizeof(CoordinatesSet) * 4, 0, GL_STATIC_DRAW));
   ...
   GL_CHECK(glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), 0));
...
void VideoViewer_Internal::create_textures(int input_width, int input_height, int output_width, int output_height)
{
   ...
   GL_CHECK(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, input_width, input_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL));
   GL_CHECK(glBindTexture(GL_TEXTURE_2D, 0));
   ...
   GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR));
   GL_CHECK(glGenerateMipmap(GL_TEXTURE_2D));
   GL_CHECK(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, output_width, output_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL));
   GL_CHECK(glBindTexture(GL_TEXTURE_2D, 0));
}
...
void VideoViewer_Internal::create_pixel_buffer_objects(uint64_t size)
{
   ...
   GL_CHECK(glBufferData(GL_PIXEL_UNPACK_BUFFER, size, 0, GL_STREAM_DRAW));
...
{
   if (m_window)
     return GLFW_CHECK(glfwWindowShouldClose(m_window));
   else
     return false;
}
...
bool VideoViewer_Internal::render_loop(int frame_rate_in_ms)
{
   ...
   while (!GLFW_CHECK(glfwWindowShouldClose(m_window)) && !m_stop){
   ...
   if (m_window)
   {
      if (!GLFW_CHECK(glfwWindowShouldClose(m_window)))
      {
         *size = m_pbo_size;
         ...
         GL_CHECK(glBufferData(GL_PIXEL_UNPACK_BUFFER, m_pbo_size, 0, GL_STREAM_DRAW));
...
bool VideoViewer_Internal::run_compute_shader(GLuint num_groups_x, GLuint num_groups_y, int input_width, int input_height)
{
   ...
   GL_CHECK(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, input_width, input_height, GL_RGBA, GL_UNSIGNED_BYTE, NULL));
   GL_CHECK(glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0));
